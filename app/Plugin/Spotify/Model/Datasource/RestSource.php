 ...
 */
	public function __construct($config, $Http = null) {
		...
		// if (!$Http) {
			...
			// Get Token
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL,            Configure::read('Spotify.url_token') );
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1 );
			curl_setopt($ch, CURLOPT_POST,           1 );
			curl_setopt($ch, CURLOPT_POSTFIELDS,     'grant_type=client_credentials' ); 
			curl_setopt($ch, CURLOPT_HTTPHEADER,     array('Authorization: Basic '.base64_encode(Configure::read('Spotify.client_id').':'.Configure::read('Spotify.client_secret')))); 
			...
			$this->baseRequest = array('header' => array(
 ...
 */
	public function read(Model $model, $queryData = array(), $recursive = null) {
		...
		if(isset($queryData['conditions'])) {
			if(isset($model->request['uri']['query']) && is_array($model->request['uri']['query'])){
				$model->request['uri']['query'] = array_merge($model->request['uri']['query'], $queryData['conditions']);
			}else {
				$model->request['uri']['query'] = $queryData['conditions'];
			}
		}
		if(isset($queryData['limit'])) {
			$model->request['uri']['query']['limit'] = $queryData['limit'];
		}
 ...
 */
	public function request($model) {
		...
		$request = array_merge($request, $this->baseRequest);
